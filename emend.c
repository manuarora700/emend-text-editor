...
#define EMEND_VERSION "1.0"
#define EMEND_TAB_STOP 8
#define EMEND_QUIT_TIMES 3
enum editorKey {
  BACKSPACE = 127,
  ARROW_LEFT = 1000,
  ARROW_RIGHT,
...
int editorRowCxToRx(erow *row, int cx) {
  ...
  for (j = 0; j < cx; j++) {
    if (row->chars[j] == '\t')
      rx += (EMEND_TAB_STOP - 1) - (rx % EMEND_TAB_STOP);
    rx++;
  }
  return rx;
}
...
int editorRowRxToCx(erow *row, int rx) {
  ...
  for (cx = 0; cx < row->size; cx++) {
    if (row->chars[cx] == '\t')
      cur_rx += (EMEND_TAB_STOP - 1) - (cur_rx % EMEND_TAB_STOP);
    cur_rx++;
...
void editorUpdateRow(erow *row) {
  ...
  for (j = 0; j < row->size; j++)
    if (row->chars[j] == '\t') tabs++;
  free(row->render);
  row->render = malloc(row->size + tabs*(EMEND_TAB_STOP - 1) + 1);
  int idx = 0;
  for (j = 0; j < row->size; j++) {
    if (row->chars[j] == '\t') {
      row->render[idx++] = ' ';
      while (idx % EMEND_TAB_STOP != 0) row->render[idx++] = ' ';
    } else {
...
void editorProcessKeypress() {
  static int quit_times = EMEND_QUIT_TIMES;
  int c = editorReadKey();
  switch (c) {
    case '\r':
      editorInsertNewline();
